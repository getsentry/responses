...
def _strip_content_encoding(headers: "Dict[str, str]") -> "Dict[str, str]":
    """Drop a "Content-Encoding" header (case-insensitively) from a headers dict.

    ``requests`` transparently decompresses the response body before it is
    exposed via ``response.text``, so by the time the recorder captures a
    response, the body is already plain/uncompressed. A "Content-Encoding"
    header copied verbatim from the real response would therefore no longer
    match the (decompressed) body, and would cause any client honoring that
    header -- such as ``requests``/``urllib3`` during replay -- to attempt to
    decompress already-uncompressed content and fail. See #724.

    HTTP header names are case-insensitive, and HTTP/2 servers send them
    lowercase, so match without regard to case (consistent with
    ``_remove_default_headers`` below).
    """
    return {
        key: value
...
class Recorder(RequestsMock):
    ...
    def _on_request(
        ...
        **kwargs: "Any",
        ...
        # ``requests_response.text`` below is already decompressed by
        # ``requests``, so a captured "Content-Encoding" header would no
        # longer describe the body we are about to store. See #724.
        headers_values = _strip_content_encoding(headers_values)
